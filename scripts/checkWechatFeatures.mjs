...
assert(loginPage.includes("openType: 'chooseAvatar'") && loginPage.includes('type="nickname"') && loginPage.includes('进入系统') && loginPage.includes('绑定已有邮箱账户'), 'Login page must allow optional WeChat profile data and enter directly without forcing email binding')
assert(loginFunction.includes("case 'start'") && loginFunction.includes("case 'register'") && loginFunction.includes("case 'bind'") && loginFunction.includes("case 'unbind'") && loginFunction.includes('needsProfile') && loginFunction.includes('handleLegacy') && !loginFunction.includes('session_key'), 'Login function must create and authenticate first-time WeChat users without returning session_key')
...
assert(accountSettingsPage.includes("title: '绑定当前微信'") && !accountSettingsPage.includes('setUsernameLogin') && accountSettingsPage.includes('unbindWechatAccount'), 'Account settings must keep optional WeChat binding and unbinding controls')
